...
class Trainer:
    def __init__(self, data_gen: AlgorithmicDataConstructor, model_args: ModelArgs, train_args: TrainArgs):
        ...
        self.model = create_model_from_data_generator(data_gen, model_args)
        self.test_accuracy = []
    ...
    def val_dataloader(self, seed: int) -> DataLoader:
        valset = self.data_gen.create_dataset(batch_size=self.train_args.valset_size, seed=seed)
    ...
    def train(self):
        ...
        for epoch in range(self.train_args.epochs):
            ...
            with torch.inference_mode():
                ...
                accuracy_per_token = sum(num_correct_per_batch) / (self.train_args.valset_size * self.data_gen.len_label)
                # Log variables, update progress bar
